...
function Index() {
    const { data: articles = [], isLoading: isLoadingNews, error: newsError } = useQuery({
        queryKey: ["articles"],
        queryFn: fetchNews,
    });
    ...
    // Specific sections
    const politicsAndCities = articles.filter(a => a.category?.slug === "politica" || a.category?.slug === "cidades").slice(0, 3).map(mapToView);
    if (newsError) {
        return (<div className="min-h-screen bg-brand-black flex flex-col">
        <TopBar />
        <MainHeader />
    ...
    return (<div className="min-h-screen bg-brand-black font-sans text-white">
      ...
      <main className="container mx-auto px-6 py-10">
        {/* Plantão / Urgente */}
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="mb-8 md:mb-12 bg-white/5 p-3 md:p-4 rounded-3xl flex items-center gap-4 md:gap-6 overflow-hidden border-l-4 border-primary shadow-2xl backdrop-blur-md border border-white/10 hover:bg-white/10 transition-all duration-300 group">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10 mb-16 md:mb-24">
          {/* Main Hero Card */}
          {isLoadingFeatured || isLoadingNews ? (<div className="lg:col-span-8">
               <Skeleton className="aspect-video w-full rounded-3xl bg-white/5"/>
             </div>) : displayFeatured ? (<Link to="/noticia/$slug" params={{ slug: displayFeatured.slug || displayFeatured.id || "" }} className="lg:col-span-8 group cursor-pointer relative">
              <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }} className="relative aspect-video overflow-hidden rounded-[2.5rem] shadow-premium bg-white/5 border border-white/10 group/hero">
                <img src={displayFeatured.image_url || IMAGE_PLACEHOLDER} alt={displayFeatured.title} loading="eager" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105"/>
            ...
            </Link>) : (<div className="lg:col-span-8 aspect-video w-full rounded-3xl bg-white/5 flex items-center justify-center border border-dashed border-white/10">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            {isLoadingNews ? (Array(3).fill(0).map((_, i) => <Skeleton key={i} className="h-24 w-full rounded-2xl bg-white/5"/>)) : secondaryNews.length > 0 ? (secondaryNews.slice(0, 3).map((post, idx) => (<Link key={idx} to="/noticia/$slug" params={{ slug: post.slug || post.id || "" }} className="group cursor-pointer flex gap-4 p-3 rounded-3xl hover:bg-white/10 transition-all duration-300 border border-transparent hover:border-white/10 hover:shadow-premium">
                ...
                </Link>))) : (<div className="p-8 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8 flex flex-col gap-20">
                {isLoadingNews ? (<div className="space-y-8">
                    ...
                    {latestNews.length > 0 ? (<NewsGrid title="Últimas da Região" items={latestNews}/>) : (<div className="py-20 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
                    ...
                    {politicsAndCities.length > 0 && (<NewsGrid title="Cidades & Política" items={politicsAndCities}/>)}
                  </>)}
            </div>
...
function displayArticleExcerpt(article) {
    if (article.excerpt)
