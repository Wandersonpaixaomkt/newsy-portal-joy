...
import { LayoutDashboard, FileText, FolderTree, Tag, Users, Image as ImageIcon, Link as LinkIcon, Megaphone, BarChart3, UserCog, Settings, LogOut, ExternalLink, ChevronLeft, Menu, AlertTriangle } from 'lucide-react';
...
function AdminLayout() {
    ...
    const navItems = [
        { label: 'Visão Geral', icon: LayoutDashboard, to: '/admin' },
        { label: 'Notícias', icon: FileText, to: '/admin/noticias' },
        { label: 'Categorias', icon: FolderTree, to: '/admin/categorias' },
        { label: 'Tags', icon: Tag, to: '/admin/tags' },
        { label: 'Autores', icon: Users, to: '/admin/autores' },
        { label: 'Biblioteca de Mídia', icon: ImageIcon, to: '/admin/midias' },
        { label: 'Fontes', icon: LinkIcon, to: '/admin/fontes' },
        { label: 'Publicidade', icon: Megaphone, to: '/admin/publicidade' },
        { label: 'Analytics', icon: BarChart3, to: '/admin/analytics' },
        { label: 'Usuários', icon: UserCog, to: '/admin/usuarios' },
        { label: 'Configurações', icon: Settings, to: '/admin/configuracoes' },
    ];
    return (<div className="min-h-screen bg-[#0f0f0f] text-neutral-200 flex overflow-hidden">
      ...
      <aside className={`hidden lg:flex flex-col border-r border-neutral-800 transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => (<Link key={item.to} to={item.to} activeProps={{ className: 'bg-red-600/10 text-red-500 border-red-600/20' }} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-neutral-800 transition-colors border border-transparent">
        ...
        <div className="p-4 border-t border-neutral-800 space-y-1">
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-red-950/20 transition-colors text-red-500 text-sm">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-neutral-800 bg-[#0f0f0f]/80 backdrop-blur-xl flex items-center justify-between px-6 sticky top-0 z-10">
          {!ENV.ADMIN_AUTH_ENABLED && (<div className="flex items-center gap-2 text-yellow-500 text-[10px] uppercase tracking-widest font-bold">
